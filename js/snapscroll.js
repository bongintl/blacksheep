...
(function($, window, document, undefined_) {
  ...
  Plugin.prototype = {
    ...
    snapping: function() {
      ...
      var positionElement = this.options.positionElement;
      console.log(scrollElement, positionElement);
      autoscrolling = false;
      return scrollElement.off("scroll.snapscroll").on("scroll.snapscroll", function() {
        var $child, cur_position, direction;
        if (!autoscrolling) {
          cur_position = positionElement.scrollTop();
          direction = _this.getDirection(prev_position, cur_position);
          $child = _this.getTargetChild($children, direction, cur_position);
          if ($child) {
            clearTimeout(timer);
            timer = setTimeout(function() {
              scrollElement.scrollTo($child, scroll_speed, scroll_options);
              $child.siblings(".ss-active").removeClass("ss-active");
              $child.addClass("ss-active");
              autoscrolling = true;
              return setTimeout(function() {
                prev_position = positionElement.scrollTop();
